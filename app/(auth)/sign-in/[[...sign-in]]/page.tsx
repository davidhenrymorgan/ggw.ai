  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-1/2 -left-1/4 h-[600px] w-[600px] rounded-full bg-brand-purple/30 blur-3xl" />
        <div className="absolute -bottom-1/2 -right-1/4 h-[600px] w-[600px] rounded-full bg-brand-orange/30 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 py-12">
          ...
          <SignIn
            appearance={{
              elements: {
                ...
                formFieldInput:
                  "bg-white/10 border-white/20 text-white rounded-xl focus:ring-2 focus:ring-white/40",
        ...
        >
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="h-32 w-24 rounded-2xl bg-gradient-card-purple opacity-70 shadow-lg"
              style={{
                transform: `rotate(${(i - 3) * 5}deg) translateY(${Math.abs(i - 3) * 10}px)`,
              }}
            />
          ))}
        </motion.div>
